...
- `Sources/VuuroScanApp.swift` — entry point; wires identity intake (real occupied/
  consent step, no more hardcoded identity) → device-capability check → one or more
  guided room captures stitched onto the same scan session (Phase 2's "unit story") →
  optional notes/photo-URL attachments → result display. There is still no property/
  unit picker or Vuuro auth — `IdentityIntakeScreen` is manual text-field entry, one
  deliberate step up from a hardcoded constant, not a substitute for real Vuuro
  account wiring (that stays out of scope until API coupling per CLAUDE.md).
- `Sources/Capture/IdentityIntakeScreen.swift` — the property/unit/org + purpose +
  occupied/consent form. Mirrors the Scan Service's own `403 consent_required` gate
  client-side so a user can't reach the capture screen for an occupied unit without
...
- `Sources/Networking/ScanServiceClient.swift` — HTTP client for the Scan Service
  ...
  (`../docs/adr/0003-privacy-acl-session-tokens.md`). Now also covers `addPhoto`/
  `addNote` (Phase 2) — `addPhoto` takes a URL only, matching the Scan Service's own
  "no image byte upload target yet" limit (`../scan-service/README.md` "Known limits");
  there is still no in-app image picker/upload path, so this is wired but unexercised
  by the manual smoke-test flow.
...
6. ~~`VuuroScanApp.swift`'s placeholder identity hardcodes `occupied: false`~~ — done:
   `IdentityIntakeScreen` now requires a real occupied/consent answer before a session
   can be created. Still confirm the SwiftUI `Form`/`Toggle` behaviour once this is
   actually built — untested like everything else here.
7. `ScanFlowView`'s multi-room loop (`RoomCaptureFlowStep`, `AnotherRoomPromptView` in
   `VuuroScanApp.swift`) reuses one `ScanSessionResponse` across repeated room
   captures and forces a fresh `CaptureCoordinator` per attempt via `.id(attempt)` on
   the enclosing view. Confirm in Xcode that `RoomCaptureSession`/`RoomCaptureView`
   actually tolerate being torn down and recreated this way for room 2, 3, etc. — this
   is the single biggest unverified assumption added on top of the original single-room
   flow, since Apple's own guided-capture sample code (as documented) is single-room.
8. Floor plan PNG/PDF export download (`GET .../export/floorplan.png|pdf`) is not wired
   into the app at all yet — only the web-viewer (`../web-viewer/`) exercises it today.
   Worth adding a "Download floor plan" action on `ResultSummaryView` once the rest of
